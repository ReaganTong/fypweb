...
function loadMgScenario() {
  ...
  if(displayLevel) displayLevel.textContent = `(EMAIL ${currentMgLevel + 1} OF ${mgScenarios.length})`;
  ...
  if(mgBody) mgBody.innerHTML = emailHtml;
}
    ...
    if (currentMgLevel < mgScenarios.length - 1) {
        feedback.innerHTML = `
          <div style="margin-bottom: 10px;">
            <span style="color:var(--neon2); font-weight:bold; font-size:1.1rem;">EMAIL SECURED!</span> All phishing indicators identified. You earned +75 Bonus XP!
          </div>
          <button class="btn-primary" style="padding:10px 20px; font-size:0.7rem; background:var(--neon2);" onclick="nextMgScenario()">NEXT EMAIL →</button>
        `;
    } else {
        feedback.innerHTML = `
          <div style="margin-bottom: 10px;">
            <span style="color:var(--neon2); font-weight:bold; font-size:1.1rem;">SIMULATION COMPLETE!</span> You are a master inspector. You earned +75 Bonus XP!
          </div>
          <button class="btn-primary" style="padding:10px 20px; font-size:0.7rem; background:var(--neon2);" onclick="closeMinigame()">FINISH SIMULATION</button>
        `;
    }
  }
}
